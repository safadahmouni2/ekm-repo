 ...
 * @version $Revision: 1.43 $ $Date: 2012/11/28 10:38:38 $
 */
public class ImportXML {

	private final Log log = LogManager.get().getLog(ImportXML.class);

	private String appUrl = "/conf/applicationContext.xml";

	private String serUrl = "/conf/servicesContext.xml";

	private String daoUrl = "/conf/daosContext.xml";

	private String busUrl = "/conf/businessContext.xml";

	private String[] ctxFiles = new String[] { appUrl, serUrl, daoUrl, busUrl };
	...
	private String errorText = "errors :\n";
	...
	public void init() {
		applicationContext = new ClassPathXmlApplicationContext(ctxFiles);
		xmlImportPath = (String) applicationContext.getBean("xmlImportPath");
		xmlImportService = (XMLImportService) applicationContext.getBean("xmlImportTxService");
	}
	...
	public static void main(String argv[]) {
		...
		importer.logger().info("START IMPORT");
		importer.init();
		importer.logger().info("INIT DB DONE");
		importer.fillCostAttributeMap();
		importer.fillComponentTypeMap();
		importer.fillComponentClassMap();
		importer.logger().info("FILL MAPS");
		File folder = new File(importer.xmlImportPath + "/");
		File[] listOfFiles = folder.listFiles();

		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile() && listOfFiles[i].getName().endsWith(".xml")) {
				System.out.println("File " + listOfFiles[i].getName());
				importer.logger().info("File " + listOfFiles[i].getName());
				String fileName = importer.xmlImportPath + "/" + listOfFiles[i].getName();
				try {
					...
					// read folderNodes
					importer.readFolderTag(doc);
					importer.removeUnusedFolders();
					importer.updateFolders();
					// read componentNodes
					BrandModel owner = importer.getOwner();
					LibraryModel lib = importer.getLib();

					for (Entry<String, EnumComponentClassModel> entry : importer.componentClassMap.entrySet()) {
						importer.readComponentTag(doc, entry.getKey(), entry.getValue(), owner, lib);
					}

				} catch (Exception e) {
					e.printStackTrace();
					importer.errorText = importer.errorText + "--- " + e + "\n";
					i++;
					importer.logger().info(e);
				}
			}
			Writer output = null;
			...
			try {
				...
				output.write(importer.errorText);
				output.close();
			} catch (IOException e) {
				e.printStackTrace();
				importer.logger().info(e);
		...
		printComponentNumberImported();
		importer.logger().info(ImportXML.savedComponent);
		System.exit(0);
	}

	public static List<String> getTagValue(String sTag, Element element) {
	...
	public static List<String> getTagNumberValue(String sTag, Element element) {
		List<String> returnList = new ArrayList<String>();
		for (int j = 0; j < element.getElementsByTagName(sTag).getLength(); j++) {
			NodeList nlList = element.getElementsByTagName(sTag).item(j).getChildNodes();
			if (nlList.getLength() > 0) {
	...
	private BaseDateTime exportDateToBaseDateTimeConverter(String dateString) {
		...
		if (dateString != null) {
			...
			try {
				...
				System.out.println("Enable to parse date: " + dateString);
					...
					if (eElement != null) {
						...
						if (!foldersMap.containsKey(folderNodeId)) {
							...
							if (VW_RESBIB_ROOT_FOLDER.equals(folderNodeId)
								...
								if (dbFolder != null) {
									System.out.println(
											"--------------------- : Same External Id" + dbFolder.getDesignation());
			...
			for (int i = 0; i < xmlFolder.getFolderChilds().size(); i++) {
				...
				if (foldersMap.containsKey(xmlFolderKey)) {
					...
					try {
						...
						errorText = errorText + "error update folder with nodeId=" + xmlFolderKey + " :" + e + "\n";
						log.info("error update folder with nodeId=" + xmlFolderKey + " :" + e);
					}
				}
			}
		}
	...
	public void readComponentTag(Document doc, String nodeName, EnumComponentClassModel enumComponentClassModel,
		...
		errorText = errorText + "---------------------------- reading tag : " + nodeName + "\n";
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (eElement != null) {
					...
					if (!getTagValue("Pos", eElement).get(0).equals("") && folderModel != null) {
						...
						String user = null;
						String date = null;
						// componentNumber = getTagValue("Pos", eElement).get(0);
						...
						componentModel.setComponentNumber(getTagValue("Pos", eElement).get(0));
						...
						if (objectType == null) {
							// throw new Exception("Objekttyp : " + componentType + " is not defined in data base");
							return;
						}
						...
						try {
							...
							e.printStackTrace();
							System.out.println("Same BS Exist: " + componentModel.getNumberAndDesignation());
							log.info("Same BS Exist: " + componentModel.getNumberAndDesignation());
							errorText = errorText + "Same BS exist for: " + componentModel.getNumberAndDesignation()
									+ "\n";
						}
					} else {
						errorText = errorText + "---component number is null for: " + componentNodeId + "\n";
					}
				}
			}
		}
	...
	private static void printComponentNumberImported() {
		System.out.println("--------------------------------------------------------------------------------------");
		System.out.println("");
		System.out.println("                        COMPONENT IMPORTED :  " + savedComponent
				+ "                                        ");
		System.out.println("");
		System.out.println("--------------------------------------------------------------------------------------");
