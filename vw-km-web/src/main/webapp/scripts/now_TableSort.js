...
Object.extend(TableKit, {
	...
	options : {
		...
		resizable : true,
		editable : true,
        rowEvenClass : 'odd', // rmo vorher even
        rowOddClass : 'even', //rmo vorher rowodd
		sortableSelector : ['table.sortable'],
		columnClass : 'sortcol',
		descendingClass : 'sortdesc',
		ascendingClass : 'sortasc',
		defaultSortDirection : -1, //rmo, vorher 1
		noSortClass : 'nosort',
	...
	reloadTable : function(table){
	  ...
	  TableKit.unloadTable(table);
 	  var op = TableKit.option('sortable', table.id); // rmo: Nutzt tableSettings
	//var op = TableKit.option('sortable resizable editable', table.id);	  if(op.sortable && table.hasClassName('sortable')) {TableKit.Sortable.init(table);}
//if(op.sortable && table.hasClassName('sortable')) {TableKit.Sortable.init(table);}
...
TableKit.Sortable = {
    init: function(elm, options) {
        ...
        if (IsDefined(table.down("thead")))
        {
//          options.rowEvenClass =  'even';
//          options.rowOddClass =  'odd';
        }
...
TableKit.Sortable.addSortType(
    ...
    new TableKit.Sortable.Type('date-newada',{
        pattern : /^\d{2}\.\d{2}\.\d{4}/i,
        normal : function(v) { 
            if(!this.pattern.test(v)) {return 0;}
            var r = v.match(/^(\d{2})\.(\d{2})\.(\d{4})/);
