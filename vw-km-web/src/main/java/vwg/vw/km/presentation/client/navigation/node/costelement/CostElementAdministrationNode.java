 ...
 * @version $Revision: 1.57 $ $Date: 2014/01/29 16:20:49 $
 */
public class CostElementAdministrationNode extends ElementFolderNode {

	private final Log log = LogManager.get().getLog(CostElementAdministrationNode.class);

	CostElementSearchFolderNode costElementSearchFolderNode = new CostElementSearchFolderNode(treeBean, this);

	public CostElementAdministrationNode(NavigationBean tree, FolderNode parent) {
		...
		setActiveFilter(treeBean.getUserFromSession().isActiveElementFilter());
	}
	...
	public List<Node> getChilds() {
		...
		for (FolderModel model : folders) {
			...
			if (user.getShowEmptyFolders() != null && !user.getShowEmptyFolders() && node.isEmptyFolder(user)) {
				log.info("Element Folder " + model.getFolderId() + " is empty");
