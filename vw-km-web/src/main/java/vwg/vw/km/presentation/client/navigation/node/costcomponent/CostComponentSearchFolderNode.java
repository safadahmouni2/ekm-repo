 ...
 * @version $Revision: 1.21 $ $Date: 2013/12/19 15:25:49 $
 */
public class CostComponentSearchFolderNode extends StaticFolderNode {

	CostComponentInProcessNode costComponentInProcessNode = new CostComponentInProcessNode(treeBean, this);

	CostComponentToBeApprovedNode costComponentToBeApprovedNode = new CostComponentToBeApprovedNode(treeBean, this);

	CostComponentSearchResultNode costComponentSearchResultNode = new CostComponentSearchResultNode(treeBean, this);

	CostComponentInactiveNode costComponentInactiveNode = new CostComponentInactiveNode(treeBean, this, -47L);
	...
	public List<Node> getChilds() {
		...
		childs.add(costComponentInProcessNode);
		childs.add(costComponentToBeApprovedNode);
		childs.add(costComponentSearchResultNode);
		childs.add(costComponentInactiveNode);
		return childs;
	}

	public CostComponentSearchFolderNode(NavigationBean tree, FolderNode parent) {
		super(tree, parent, StaticNodeIdEnum.COST_COMPONENT_SEARCH_FOLDER_NODE.value());
		treeBean.getObjectTypeSearchResultNode().put(getObjectContentId(), costComponentSearchResultNode);
		Map<Long, StaticFolderNode> statusNodes = new HashMap<Long, StaticFolderNode>();
		statusNodes.put(EnumStatusManager.Status.IN_PROGRESS.value(), costComponentInProcessNode);
		statusNodes.put(EnumStatusManager.Status.CREATED.value(), costComponentInProcessNode);
		statusNodes.put(EnumStatusManager.Status.TO_BE_APPROVED.value(), costComponentToBeApprovedNode);
