 ...
 * @version $Revision: 1.34 $ $Date: 2014/03/06 13:54:58 $
 */
public class CostElementInProcessNode extends SearchFolderNode {

	private final Log log = LogManager.get().getLog(CostElementInProcessNode.class);

	public CostElementInProcessNode(NavigationBean tree, FolderNode parent) {
		super(tree, parent, StaticNodeIdEnum.COST_ELEMENT_IN_PROCESS_NODE.value());
	...
	public List<Node> getChilds() {
		log.info("In process node clicked");
		...
		for (ElementModel model : elements) {
			...
			if (loggedUser.haveRole(RoleManager.Role.ADMINISTRATOR.value())
					&& loggedUser.getUserBrands().contains(model.getOwner())) {
