 ...
 * @version $Revision: 1.33 $ $Date: 2016/11/01 10:04:42 $
 */
public class WorkAreaAdministrationNode extends StaticFolderNode {

	WorkAreaSearchResultNode workAreaSearchResultNode = new WorkAreaSearchResultNode(treeBean, this);

	public WorkAreaAdministrationNode(NavigationBean tree, FolderNode parent) {
		super(tree, parent, 2L);
		setContextMenu(ContextMenuEnum.StaticFolderContextMenu);
		setTemplateName("./administration/workAreaAdministration/workAreaList.xhtml");
		setAddObjectIcon("add-work-area");
		treeBean.getObjectTypeSearchResultNode().put(getObjectContentId(), workAreaSearchResultNode);
	}
	...
	public List<Node> getChilds() {
		...
		childs.add(workAreaSearchResultNode);
		// load work area
		WorkAreaDTO dto = new WorkAreaDTO();
		// set sort order
		dto.setAscending(false);
		List<WorkAreaModel> list = treeBean.getWorkAreaService().loadWorkAreas(dto).getWorkAreas();
		setDto(dto);
		for (WorkAreaModel workAreaModel : list) {
			WorkAreaNode n = new WorkAreaNode(treeBean, this, workAreaModel.getDesignation(),
					workAreaModel.getWorkAreaId(), Boolean.FALSE, workAreaModel.getBrandIcon());
			childs.add(n);
			objectChildsCount++;
	 ...
	 */
	public WorkAreaNode getNewObjectNodeInstance() {
		return new WorkAreaNode(treeBean, this, treeBean.getMessage("workarea.newWorkarea.tree.msge"), null,
				Boolean.FALSE, null);
	}
	...
	protected String getListClickedMessage() {
		return treeBean.getMessage("action.workarea.list");
	}
	...
	protected String getCreateMessage() {
		return treeBean.getMessage("action.workArea.create");
	}
}
